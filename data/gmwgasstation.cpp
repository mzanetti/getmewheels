...
GMWItem::Type GMWGasStation::objectType() const
{
    return GMWItem::TypeGasStation;
}

QDataStream &operator<<(QDataStream& stream, const GMWGasStation& gasStation)
{
    stream << dynamic_cast<const GMWItem&>(gasStation);
    return stream;
}

QDataStream &operator>>(QDataStream& stream, GMWGasStation& gasStation)
{
    stream >> dynamic_cast<GMWItem&>(gasStation);
    return stream;
}
