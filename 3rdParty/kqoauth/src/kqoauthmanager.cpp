...
void KQOAuthManager::executeRequest(KQOAuthRequest *request) {
    ...
    // OK... now this is dirty... car2go API somehow refuses all our signatures if
    // they contain a + and I don't want to waste more days getting kqoauth to work.
    // Just generate a new one with a new nonce until we have one without + in it...
    bool sigOK = false;
    int runs = 0;

    while(!sigOK) {
//        sigOK = true; // This disables the + workaround...
        authHeader.clear();
        runs++;
        ...
        foreach (const QByteArray header, requestHeaders) {
            ...
            if(header.startsWith("oauth_signature=")) {
                qDebug() << "checking header:" << header << QByteArray::fromPercentEncoding(header);
                if(!QByteArray::fromPercentEncoding(header).contains('+')) {
                    sigOK = true;
                }
            }
    ...
    qDebug() << "got valid signature after" << runs << "tries";
...
//        urlWithParams.setQueryItems(urlParams);
        ...
        qDebug() << "KQOauth: getting" << networkRequest.url() << networkRequest.rawHeader("Authorization") ;
        //<< urlParams << urlWithParams << request->requestParameters() << request->additionalParameters();
        d->networkManager->get(networkRequest);
    } else if( request->httpMethod() == KQOAuthRequest::POST) {
        networkRequest.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded");
        qDebug() << "POST:" << networkRequest.url() << QByteArray::fromPercentEncoding(authHeader);
        d->networkManager->post(networkRequest, QByteArray::fromPercentEncoding(authHeader));
        ...
        d->networkManager->sendCustomRequest(networkRequest, "DELETE");
    }
}
...
void KQOAuthManager::onRequestReplyReceived( QNetworkReply *reply ) {
    ...
    qDebug() << "KOauth:" << "got response" << networkReply << reply->errorString() << reply->rawHeader("WWW-Authenticate");

    // Just don't do anything if we didn't get anything useful.
//    if(networkReply.isEmpty() && error) {
//        reply->deleteLater();
//        return;
//    }
