                                                        ...
                                                        const QByteArray &tokenSecret, const ParamMap &params )
{
    ...
    if ( httpMethod == GET ) {
        ...
        qDebug() << "requesting:" << authorizationHeader;
    }
    ...
    if ( httpMethod == GET ) {
        ...
        qDebug() << "posting:" << request.url() << request.rawHeader("application/x-www-form-urlencoded") << authorizationHeader;
        reply = manager->post( request, authorizationHeader );
    }

    // start the event loop and wait for the response
    loop->exec();
    qDebug() << reply->readAll() << reply->rawHeader("WWW-Authenticate");
                                                      ...
                                                      const QByteArray &tokenSecret, ParamMap *params )
{
    ...
    if ( signatureMethod == PLAINTEXT ) {
        ...
        if ( signatureMethod == HMAC_SHA1 ) {
            ...
            qDebug() << "using tokenSecret:" << tokenSecret;
            qDebug() << "using consumerSecret:" << consumerSecret;
            QByteArray key( consumerSecret.toPercentEncoding() + "&" + tokenSecret.toPercentEncoding() );
//            QByteArray key( consumerSecret.toPercentEncoding() + "&" + QByteArray::fromPercentEncoding(tokenSecret).toPercentEncoding() );
            qDebug() << "using key:" << key;
            // create HMAC-SHA1 digest in Base64
            QCA::MessageAuthenticationCode hmac( "hmac(sha1)", QCA::SymmetricKey( key ) );
            qDebug() << "signing:" << signatureBaseString;
